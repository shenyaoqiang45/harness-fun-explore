...
import { createSerialGate, fetchWith429Retry } from "./evidence-rate-limit.js";

const S2_BASE = "https://api.semanticscholar.org/graph/v1";
const S2_AUTHORITY = 0.9;
const S2_INTERVAL_WITH_KEY_MS = 1200;
const S2_INTERVAL_ANONYMOUS_MS = 4000;
const S2_RETRY_DELAY_WITH_KEY_MS = 1200;
const S2_RETRY_DELAY_ANONYMOUS_MS = 4000;

let sharedSerialGate: ReturnType<typeof createSerialGate> | null = null;
let sharedSerialGateIntervalMs = -1;

function getSharedSerialGate(intervalMs: number) {
  if (!sharedSerialGate || sharedSerialGateIntervalMs !== intervalMs) {
    sharedSerialGate = createSerialGate(intervalMs);
    sharedSerialGateIntervalMs = intervalMs;
  }
  return sharedSerialGate;
}
...
export interface SemanticScholarEvidenceOptions {
  ...
  timeoutMs?: number;
  apiKey?: string;
  /** Minimum gap between S2 requests. 0 disables throttling (tests). */
...
function resolveMinIntervalMs(apiKey: string | undefined, override?: number): number {
  if (override !== undefined) {
    return override;
  }
  return apiKey ? S2_INTERVAL_WITH_KEY_MS : S2_INTERVAL_ANONYMOUS_MS;
}
...
function buildEvidence(keyword: string, payload: SemanticScholarSearchResponse): EvidenceItem[] {
...
export function createSemanticScholarEvidenceProvider(
  ...
  const minIntervalMs = resolveMinIntervalMs(apiKey, options.minIntervalMs);
  const minRetryDelayMs = apiKey ? S2_RETRY_DELAY_WITH_KEY_MS : S2_RETRY_DELAY_ANONYMOUS_MS;
  const runQueued =
    minIntervalMs === 0
      ? <T>(task: () => Promise<T>) => task()
      : getSharedSerialGate(minIntervalMs);
